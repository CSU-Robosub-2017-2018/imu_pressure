...
########################################################################################################################
    ...
    def postProcessing(self, dataPos, dataR, fileName='youForgotToNameYourFile',
                           save_path='C:/Users/bob/Desktop/IMU/imu_framework/tests/test_files/'):
        ...
        data = [dataPos[:,0], dataPos[:,1], dataPos[:,2],
                dataR[:,0], dataR[:,1], dataR[:,2],
                dataR[:,3], dataR[:,4], dataR[:,5],
                dataR[:,6], dataR[:,7], dataR[:,8]]
    ...
    def getAccBank(self):
        ouptut = self.memData.getAccBank()
        return ouptut